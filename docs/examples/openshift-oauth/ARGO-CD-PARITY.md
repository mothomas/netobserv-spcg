...
| `clientID` + `clientSecret` in `argocd-secret` (operator-generated) | `OAUTH_CLIENT_ID` + secret **`spcg-oauth-client`** via Job **`spcg-oauth-bootstrap`** (`manifests/openshift-secure/oauth-bootstrap/`) |
